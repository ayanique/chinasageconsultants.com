...
function dzsap_woo_product_write_panel() {
	...
	if($dzsap->mainoptions['skinwave_wave_mode']!='canvas') {
        ...
        <p class="form-field dzsap_woo_product_track_field "><label for="dzsap_woo_product_track"><?php echo esc_html__("Waveform", 'dzsap'); ?></label>
            <span class="regenerate-waveform-con">
            <button class="button-secondary regenerate-waveform regenerate-waveform--from-woo " data-playerid="<?php echo $_GET['post']; ?>"><?php echo esc_html__("Regenerate Waveform", 'dzsap'); ?></button>
        </span> </p>
	...
	woocommerce_wp_text_input( array( 'id' => $lab, 'class'=>'sample-time-start-feeder', 'label' => __( 'Sample Time Start' ), 'description' => esc_html__('If this is a sample ( you are not showing the full track, you can input a start time here ), leave 0 if not','dzsap'), 'value' => $val ) );
...
?><p class="form-field dzsap_woo_product_track_field "><label for="dzsap_woo_product_track"><?php echo esc_html__("Playlist", 'dzsap'); ?></label>
            ...
            if(DZSZoomSoundsHelper::check_playlist_exists('zoomsounds-product-playlist-'.$_GET['post'])){
              echo ' dsz-auto-click-after-1000';
            }
