<?php
namespace AllstonCore\CPT\Shortcodes\IconWithText;
...
class IconWithText implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Icon With Text', 'allston-core' ),
					'base'                      => $this->base,
	...
	public function render( $atts, $content = null ) {
		...
		$params['svg_holder_data'] = $this->getSVGDataAttrs( $params );
		...
		return allston_core_get_shortcode_module_template_part( 'templates/iwt', 'icon-with-text', $params['type'], $params );
	}
	
	private function getIconParameters( $params ) {
		$params_array = array();

		$params_array['icon_source']   = $params['icon_source'];

		if ( empty( $params['custom_icon'] ) && $params['icon_source'] == 'icon-font') {
			...
			if ( ! empty( $params['custom_icon_size'] ) ) {
				$params_array['custom_size'] = allston_eltdf_filter_px( $params['custom_icon_size'] ) . 'px';
			}
			...
			if ( ! empty( $params['shape_size'] ) ) {
				$params_array['shape_size'] = allston_eltdf_filter_px( $params['shape_size'] ) . 'px';
			}
			...
			if ( $params['icon_border_width'] !== '' ) {
				$params_array['border_width'] = allston_eltdf_filter_px( $params['icon_border_width'] ) . 'px';
			}
			...
			$params_array['svg_path']   = $params['svg_path'];
		}
	...
	private function getHolderClasses( $params ) {
		...
		$holderClasses[] = ! empty( $params['icon_source'] ) ? 'eltdf-iwt-' . esc_attr( $params['icon_source'] ) : '';
	...
	private function getSVGDataAttrs($params) {
		$svgData = array();

	    if (!empty($params['svg_path'])) {
			$svgData['data-svg-animation-duration'] = $params['svg_animation_duration'];
			$svgData['data-svg-animation-delay'] = $params['svg_animation_delay'];
	    }
	...
	private function getContentStyles( $params ) {
		...
		if ( $params['text_padding'] !== '' && $params['type'] === 'icon-left' ) {
			$styles[] = 'padding-left: ' . allston_eltdf_filter_px( $params['text_padding'] ) . 'px';
		}
		
		if ( $params['text_padding'] !== '' && $params['type'] === 'icon-top' ) {
			$styles[] = 'padding-top: ' . allston_eltdf_filter_px( $params['text_padding'] ) . 'px';
		}
	...
	private function getTitleStyles( $params ) {
		...
		if ( $params['title_top_margin'] !== '' ) {
			$styles[] = 'margin-top: ' . allston_eltdf_filter_px( $params['title_top_margin'] ) . 'px';
		}
	...
	private function getTextStyles( $params ) {
		...
		if ( $params['text_top_margin'] !== '' ) {
			$styles[] = 'margin-top: ' . allston_eltdf_filter_px( $params['text_top_margin'] ) . 'px';
		}
