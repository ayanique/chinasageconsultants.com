...
namespace AllstonCore\CPT\Shortcodes\FullscreenPortfolioGrid;

use AllstonCore\Lib;

class FullscreenPortfolioGrid implements Lib\ShortcodeInterface {
	private $base;

	public function __construct() {
		$this->base = 'eltdf_fullscreen_portfolio_grid';

		add_action( 'vc_before_init', array( $this, 'vcMap' ) );

		//Portfolio category filter
		add_filter( 'vc_autocomplete_eltdf_fullscreen_portfolio_grid_category_callback', array( &$this, 'portfolioCategoryAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

		//Portfolio category render
		add_filter( 'vc_autocomplete_eltdf_fullscreen_portfolio_grid_category_render', array( &$this, 'portfolioCategoryAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array

		//Portfolio selected projects filter
		add_filter( 'vc_autocomplete_eltdf_fullscreen_portfolio_grid_selected_projects_callback', array( &$this, 'portfolioIdAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

		//Portfolio selected projects render
		add_filter( 'vc_autocomplete_eltdf_fullscreen_portfolio_grid_selected_projects_render', array( &$this, 'portfolioIdAutocompleteRender', ), 10, 1 ); // Render exact portfolio. Must return an array (label,value)

		//Portfolio tag filter
		add_filter( 'vc_autocomplete_eltdf_fullscreen_portfolio_grid_tag_callback', array( &$this, 'portfolioTagAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

		//Portfolio tag render
		add_filter( 'vc_autocomplete_eltdf_fullscreen_portfolio_grid_tag_render', array( &$this, 'portfolioTagAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array
	}
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map( array(
					'name'     => esc_html__( 'Fullscreen Portfolio Grid', 'allston-core' ),
					'base'     => $this->getBase(),
					'category' => esc_html__( 'by ALLSTON', 'allston-core' ),
					'icon'     => 'icon-wpb-fullscreen-portfolio-grid extended-custom-icon',
					'params'   => array(
						array(
							'type'        => 'dropdown',
							...
							'description' => esc_html__( 'Default value is Four', 'allston-core' ),
							'admin_label' => true
						),
						array(
							'type'       => 'dropdown',
						...
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'category',
							'heading'     => esc_html__( 'One-Category Portfolio Grid', 'allston-core' ),
						...
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'tag',
							'heading'     => esc_html__( 'One-Tag Portfolio Grid', 'allston-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$html = allston_core_get_cpt_shortcode_module_template_part( 'portfolio', 'fullscreen-portfolio-grid', 'fullscreen-grid-holder', '',$params, $additional_params );
	...
	public function getHolderClasses( $params ) {
		...
		switch ( $number_of_columns ):
			case '1':
				$classes[] = 'eltdf-fpg-one-column';
				break;
			case '2':
				$classes[] = 'eltdf-fpg-two-columns';
				break;
			case '3':
				$classes[] = 'eltdf-fpg-three-columns';
				break;
			case '4':
				$classes[] = 'eltdf-fpg-four-columns';
				break;
			case '5':
				$classes[] = 'eltdf-fpg-five-columns';
				break;
			default:
				$classes[] = 'eltdf-fpg-four-columns';
				break;
