...
class AllstonEltdfIconWidget extends AllstonEltdfWidget {
	public function __construct() {
		parent::__construct(
			'eltdf_icon_widget',
			esc_html__( 'Elated Icon Widget', 'allston' ),
			array( 'description' => esc_html__( 'Add icons to widget areas', 'allston' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			allston_eltdf_icon_collections()->getIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'icon_text',
					'title' => esc_html__( 'Icon Text', 'allston' )
				),
				array(
					'type'  => 'textfield',
					'name'  => 'link',
				...
				array(
					'type'    => 'dropdown',
					'name'    => 'target',
					'title'   => esc_html__( 'Target', 'allston' ),
	...
	public function widget( $args, $instance ) {
		...
		$data   = array();
		$data[] = ! empty( $instance['icon_hover_color'] ) ? allston_eltdf_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
		$data   = implode( ' ', $data );
		...
		if ( ! empty( $instance['icon_size'] ) ) {
			$holder_styles[] = 'font-size: ' . allston_eltdf_filter_px( $instance['icon_size'] ) . 'px';
		}
		...
		if ( ! empty( $instance['icon_text'] ) ) {
			$icon_text_html = '<h5 class="eltdf-icon-text ' . esc_attr( $icon_text_class ) . '">' . esc_html( $instance['icon_text'] ) . '</h5>';
		}
		?>
		
		<a <?php allston_eltdf_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo allston_eltdf_get_inline_style( $holder_styles ); ?>>
