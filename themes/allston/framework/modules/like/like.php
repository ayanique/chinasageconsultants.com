...
class AllstonEltdfLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_allston_eltdf_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_allston_eltdf_like', array( $this, 'ajax' ) );
	}
	...
	public function like_post( $post_id, $action = 'get', $type = '' ) {
		...
		switch ( $action ) {
			...
			case 'get':
				...
				$return_value =  "<span>" . esc_attr( $like_count ) . "</span>" .$icon ;
			...
			case 'update':
				...
				$return_value = "<span>" . esc_attr( $like_count ) . "</span><i class='icon_heart' aria-hidden='true'></i>";
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'allston' );
		
		if ( isset( $_COOKIE[ 'eltdf-like_' . $post->ID ] ) ) {
			$class = 'eltdf-like liked';
			$title = esc_html__( 'You already like this!', 'allston' );
		}
...
if ( ! function_exists( 'allston_eltdf_create_like' ) ) {
	function allston_eltdf_create_like() {
		AllstonEltdfLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'allston_eltdf_create_like' );
}