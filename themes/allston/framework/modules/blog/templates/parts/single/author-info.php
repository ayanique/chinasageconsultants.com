<?php
$author_info_box       = esc_attr( allston_eltdf_options()->getOptionValue( 'blog_author_info' ) );
$author_info_email     = esc_attr( allston_eltdf_options()->getOptionValue( 'blog_author_info_email' ) );
$author_id             = esc_attr( get_the_author_meta( 'ID' ) );
$social_networks       = allston_eltdf_core_plugin_installed() ? allston_eltdf_get_user_custom_fields() : false;
$display_author_social = allston_eltdf_options()->getOptionValue( 'blog_single_author_social' ) === 'no' ? false : true;
?>
			...
			<div class="eltdf-author-description-content">
				<div class="eltdf-author-description-image">
					<a itemprop="url" href="<?php echo esc_url( get_author_posts_url( $author_id ) ); ?>" title="<?php the_title_attribute(); ?>">
						<?php echo allston_eltdf_kses_img( get_avatar( get_the_author_meta( 'ID' ), 78 ) ); ?>
					</a>
				</div>
							...
							<div class="eltdf-author-social-icons clearfix">
								<?php foreach ( $social_networks as $network ) { ?>
									<a itemprop="url" href="<?php echo esc_url( $network['link'] ) ?>" target="_blank">
										<?php echo allston_eltdf_icon_collections()->renderIcon( $network['class'], 'font_elegant' ); ?>
									</a>
								<?php } ?>
							</div>
						<?php } ?>
