...
/*
   Class: AllstonEltdfDashboardForm
   A class that initializes Elated Dashboard Container
*/
class AllstonEltdfDashboardForm implements iAllstonEltdfLayoutNode, iAllstonEltdfRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
...
/*
   Class: AllstonEltdfDashboardGroup
   A class that initializes AllstonEltdf Group Field
*/
class AllstonEltdfDashboardGroup implements iAllstonEltdfLayoutNode, iAllstonEltdfRender{
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: AllstonEltdfDashboardTitle
   A class that initializes Dashboard Title
*/
class AllstonEltdfDashboardTitle implements iAllstonEltdfRender {
	private $name;
	private $title;
...
/*
   Class: AllstonEltdfDashboardField
   A class that initializes AllstonEltdf Front Field
*/

class AllstonEltdfDashboardField implements iAllstonEltdfRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class AllstonEltdfDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class AllstonEltdfDashboardFieldText extends AllstonEltdfDashboardFieldType {
...
class AllstonEltdfDashboardFieldTextArea extends AllstonEltdfDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class AllstonEltdfDashboardFieldImage extends AllstonEltdfDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
			...
			<div class="eltdf-dashboard-gallery-holder">
				<ul class="eltdf-dashboard-gallery-images-holder">
					<?php if ( isset($value_html) ) {
						echo allston_eltdf_get_module_part($value_html);
					} ?>
				</ul>
				<div class="eltdf-dashboard-gallery-uploader">
					<?php 
					if ( allston_membership_theme_installed() ) {
						echo allston_eltdf_get_button_html( array(
							'text'      => esc_html__( 'Upload', 'allston' ),
							'custom_class' => 'eltdf-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" class="eltdf-btn eltdf-btn-medium eltdf-btn-solid eltdf-dashboard-gallery-upload"><span class="eltdf-btn-text">' . esc_html__( 'Upload', 'allston' ) . '</span></a>';
					} ?>
					...
					<?php if ($value !== '' && $value !== false) { ?>
						<button class="eltdf-btn eltdf-btn-solid eltdf-dashboard-remove-image"><?php esc_html_e('Remove Media','allston'); ?></button>
					<?php } ?>
				</div>
			</div>
		</div>
...
class AllstonEltdfDashboardFieldGallery extends AllstonEltdfDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class AllstonEltdfDashboardFieldSelect extends AllstonEltdfDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class AllstonEltdfDashboardFieldDate extends AllstonEltdfFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class AllstonEltdfDashboardFieldIcon extends AllstonEltdfDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$options           = \AllstonEltdfIconCollections::get_instance()->getIconCollectionsEmpty();
		$icons_collections = \AllstonEltdfIconCollections::get_instance()->getIconCollectionsKeys();
				...
				<div class="eltdf-dashboard-item">
					...
					<div class="eltdf-dashboard-icon-holder">
						...
						<?php foreach ( $icons_collections as $icons_collection ) {
							$icons_param = \AllstonEltdfIconCollections::get_instance()->getIconCollectionParamNameByKey( $icons_collection );
	                        $style       = 'display: none';
							...
							<div class="eltdf-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                                <select name="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>" id="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>">
									<?php
									$icons = \AllstonEltdfIconCollections::get_instance()->getIconCollection( $icons_collection );
...
class AllstonEltdfDashboardFieldColor extends AllstonEltdfDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class AllstonEltdfDashboardFieldCheckBoxGroup extends AllstonEltdfDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class AllstonEltdfDashboardFieldAddress extends AllstonEltdfFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
			...
			<div class="eltdf-dashboard-field-row">
				<div class="eltdf-dashboard-item col-lg-<?php echo esc_attr($col_width); ?>">
					<div class="eltdf-dashboard-input-holder">
						...
						<a id="reset" href="#" style="display:none;"><?php esc_html_e( 'Reset Marker', 'allston' ); ?></a>
					</div>
				</div>
			</div>
		</div>
...
class AllstonEltdfDashboardRepeater implements iAllstonEltdfRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false, $value = array() ) {
		...
		$this->name        = $name;
		$this->num_of_rows = 1;
		$this->button_text = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'allston' );
			...
			<?php if ($this->table_layout) { ?>
				<div class="eltdf-dashboard-repeater-table-heading">
					<div class="eltdf-dashboard-repeater-fields-holder">
						<div class="eltdf-dashboard-repeater-table-cell eltdf-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'allston' ) ?></div>
						...
						<div class="eltdf-dashboard-repeater-table-cell eltdf-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'allston' ) ?></div>
					</div>
				</div>
			<?php } ?>
...
class AllstonEltdfDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array()) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new AllstonEltdfDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'textarea':
				$field = new AllstonEltdfDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'date':
				$field = new AllstonEltdfDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'image':
				$field = new AllstonEltdfDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'gallery':
				$field = new AllstonEltdfDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'select':
				$field = new AllstonEltdfDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'icon':
				$field = new AllstonEltdfDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'color':
				$field = new AllstonEltdfDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'checkboxgroup':
				$field = new AllstonEltdfDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'address':
				$field = new AllstonEltdfDashboardFieldAddress();
