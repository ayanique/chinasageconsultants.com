...
/*
   Class: AllstonEltdfMultipleImages
   A class that initializes Elated Multiple Images
*/

class AllstonEltdfMultipleImages implements iAllstonEltdfRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$allston_eltdf_Framework->eltdfMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="eltdf-gallery-uploader">
								<a class="eltdf-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'allston' ); ?></a>
								<a class="eltdf-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'allston' ); ?></a>
							</div>
						</div>
					</div>
				</div>
...
class AllstonEltdfTwitterFramework implements iAllstonEltdfRender {
	public function render( $factory ) {
		$twitterApi = AllstonTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'allston' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'allston' ) : esc_html__( 'Connect with Twitter', 'allston' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="eltdf-page-form-section" id="eltdf_enable_social_share">
			<div class="eltdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'allston' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'allston' ); ?></p>
			</div>
			<div class="eltdf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class AllstonEltdfInstagramFramework implements iAllstonEltdfRender {
	public function render( $factory ) {
		$instagram_api = AllstonInstagramApi::getInstance();
		$message       = '';
		
		//if code wasn't saved to database
		if ( ! get_option( 'allston_instagram_code' ) ) {
			...
			if ( ! empty( $_GET['code'] ) ) {
				...
				$message = esc_html__( 'You have successfully connected with your Instagram account. If you have any issues fetching data from Instagram try reconnecting.', 'allston' );
		...
		$buttonText = $instagram_api->hasUserConnected() ? esc_html__( 'Re-connect with Instagram', 'allston' ) : esc_html__( 'Connect with Instagram', 'allston' );
		?>
		<?php if ( $message !== '' ) { ?>
			<div class="alert alert-success" style="margin-top: 20px;">
				<span><?php echo esc_html( $message ); ?></span>
		...
		<div class="eltdf-page-form-section" id="eltdf_enable_social_share">
			<div class="eltdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Instagram', 'allston' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'allston' ); ?></p>
			</div>
			<div class="eltdf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class AllstonEltdfRepeater implements iAllstonEltdfRender {
	private $label;
	private $description;
	private $name;
	private $fields;
			...
			<?php if ($this->table_layout) { ?>
				<div class="eltdf-repeater-table-heading">
					<div class="eltdf-repeater-fields-holder">
						<div class="eltdf-repeater-table-cell eltdf-repeater-sort"><?php esc_html_e( 'Order', 'allston' ) ?></div>
						...
						<div class="eltdf-repeater-table-cell eltdf-repeater-remove"><?php esc_html_e( 'Remove', 'allston' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="eltdf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="eltdf-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = allston_eltdf_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class AllstonEltdfFieldAddress extends AllstonEltdfFieldType {
				...
				<div class="container-fluid">
					<div class="row">
						...
						<?php endif; ?>
							...
							<button id="find" class="btn btn-primary"><?php esc_html_e( 'Place the pin on the map', 'allston' ); ?></button>
...
class AllstonEltdfFieldIcon extends AllstonEltdfFieldType {
    public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
        $class = '';
        ...
        $icon_packs        = \AllstonEltdfIconCollections::get_instance()->getIconCollectionsEmpty();
