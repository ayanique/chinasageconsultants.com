...
class AllstonEltdfFramework {
	...
	public $eltdfTaxonomyOptions;
	public $eltdfUserOptions;
	private $skin;
	
	private function __construct() {
		$this->eltdfOptions         = AllstonEltdfOptions::get_instance();
		$this->eltdfMetaBoxes       = AllstonEltdfMetaBoxes::get_instance();
		$this->eltdfTaxonomyOptions = AllstonEltdfTaxonomyOptions::get_instance();
		$this->eltdfUserOptions     = AllstonEltdfUserOptions::get_instance();
		$this->eltdfDashboardOptions = AllstonEltdfDashboardOptions::get_instance();
	}
	...
	public function setSkin( AllstonEltdfSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class AllstonEltdfSkinManager
 *
...
class AllstonEltdfSkinManager {
	...
	/**
	 * @see AllstonEltdfSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = ELATED_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'allston' );
...
/**
 * Class AllstonEltdfSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class AllstonEltdfSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( allston_eltdf_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.eltdfSCIcon = '<?php echo allston_eltdf_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.eltdfSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'allston' ) ?>';
		</script>
	<?php }
...
class AllstonEltdfOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $allston_eltdf_options ) ) {
			return $allston_eltdf_options[ $key ];
...
/*
   Class: AllstonEltdfAdminPage
   A class that initializes Elated Admin Page
*/
class AllstonEltdfAdminPage implements iAllstonEltdfLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title_label = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new AllstonEltdfFieldFactory();
		$this->slug    = $slug;
	...
	public function renderChild( iAllstonEltdfRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: AllstonEltdfMetaBoxes
...
/*
   Class: AllstonEltdfMetaBox
   A class that initializes Elated Meta Box
*/
class AllstonEltdfMetaBox implements iAllstonEltdfLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $allston_eltdf_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: AllstonEltdfTaxonomyOptions
   A class that initializes AllstonEltdf Taxonomy Options
*/
class AllstonEltdfTaxonomyOptions {
	private static $instance;
...
/*
   Class: AllstonEltdfTaxonomyOption
   A class that initializes AllstonEltdf Taxonomy Option
*/
class AllstonEltdfTaxonomyOption implements iAllstonEltdfLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new AllstonEltdfTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: AllstonEltdfUserOptions
   A class that initializes AllstonEltdf User Options
*/
class AllstonEltdfUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: AllstonEltdfUserOption
   A class that initializes AllstonEltdf User Option
*/
class AllstonEltdfUserOption implements iAllstonEltdfLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new AllstonEltdfUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: AllstonEltdfDashboardOptions
   A class that initializes AllstonEltdf Dashboard Options
*/
class AllstonEltdfDashboardOptions {
	private static $instance;
...
/*
   Class: AllstonEltdfDashboardOption
   A class that initializes AllstonEltdf Dashboard Option
*/
class AllstonEltdfDashboardOption implements iAllstonEltdfLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new AllstonEltdfDashboardFieldFactory();
	}
...
if ( ! function_exists( 'allston_eltdf_init_framework_variable' ) ) {
	function allston_eltdf_init_framework_variable() {
		global $allston_eltdf_Framework;
		
		$allston_eltdf_Framework = AllstonEltdfFramework::get_instance();
		$eltdfSkinManager                 = new AllstonEltdfSkinManager();
		$allston_eltdf_Framework->setSkin( $eltdfSkinManager->getSkin() );
	}
	
	add_action( 'allston_eltdf_before_options_map', 'allston_eltdf_init_framework_variable' );
}
?>